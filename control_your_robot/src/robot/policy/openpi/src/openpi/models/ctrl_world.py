...
from openpi.models.pipeline_stable_video_diffusion import StableVideoDiffusionPipeline
from openpi.models.pipeline_ctrl_world import CtrlWorldDiffusionPipeline
from openpi.models.unet_spatio_temporal_condition import UNetSpatioTemporalConditionModel
...
class CtrlWorld(nn.Module):
    ...
    def forward(self, batch):
        ...
        num_history = int(self.args.num_history)
        num_frames = int(self.args.num_frames)
        latents = latents.to(device)

        # Temporal contract used across train and rollout:
        # latents = [history(7 past frames), current+future(5 frames)]
        # - history part excludes current frame
        # - future part starts with current frame
        if latents.ndim != 5:
            raise ValueError(f"Expected latent tensor [B,T,C,H,W], got {tuple(latents.shape)}")
        if latents.shape[1] != num_history + num_frames:
            raise ValueError(
                f"Expected T=num_history+num_frames={num_history + num_frames}, got {latents.shape[1]}"
            )
