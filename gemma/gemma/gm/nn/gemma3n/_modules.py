...
class Attention(nn.Module):
  ...
  def __call__(
    ...
    if self.use_value_norm and kv_shared_cache is None:
...
class FeedForward(nn.Module):
  ...
  def __call__(self, x):
    ...
    # [batch_size, seq_len, features]
    return linear("...H,HF->...F", activations)
...
class PerLayerMapping(nn.Module):
  ...
  def __call__(
    ...
    return self.post_per_layer_input_norm(x)
...
class Laurel(nn.Module):
  ...
  def __call__(
    ...
    return self.linear_right("btr,rd->btd", x)
