...
class SamplerEvaluator(kd.evals.EvaluatorBase):
  ...
  def evaluate(self, state: kd.train.TrainState, step: int) -> Any:
    ...
    sharding = kd.sharding.REPLICATED if self.ds.batch_size is None else kd.sharding.FIRST_DIM
  ...
  def _get_prompt_and_image_from_batch(self, ex: Any) -> tuple[str, Any]:
    ...
    images = kd.kontext.get_by_path({"batch": ex}, self.model.images) if self.model.images is not None else None
