...
class Input:
  ...
  def length_with_mm(self) -> int:
    ...
    num_tokens_per_image = self.config.num_tokens_per_image if self.config.support_images else 0
  ...
  def attention_mask(self) -> Bool["B length_with_mm length_with_mm"]:
    ...
    bidirectional_mask = self.tokens_with_mm == _token_utils.SOFT_TOKEN_PLACEHOLDER if self.images is not None else None
