...
class MAPHead(nn.Module):
  ...
  def __call__(self, x: jax.Array) -> jax.Array:
    ...
    n, _l, d = x.shape  # pylint: disable=unused-variable
    probe = self.param(
...
class Encoder1DBlock(nn.Module):
  ...
  def __call__(
    ...
    y = nn.Dropout(rate=self.dropout)(y, deterministic)
    x = x + y
    return nn.with_logical_constraint(x, ("act_batch", "act_len", "act_emb"))
...
class ViTModel(nn.Module):
  ...
  def __call__(
    ...
    n, _l, c = x.shape  # pylint: disable=unused-variable
